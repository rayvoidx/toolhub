...
(function tool() {
  ...
  // TOOLJS:START
  var cfg = window.APP_CONFIG || {};
  var LS_KEY = (cfg.slug || "area-conv") + ":units";

  // calc-core:start — 순수 계산 코어 (node 단위검증 대상, 제곱미터 m² 기준 환산 계수)
  var FACTOR = {
    mm2: 0.000001,       // 1 mm² = 1e-6 m²
    cm2: 0.0001,         // 1 cm² = 1e-4 m²
    m2: 1,               // 기준
    km2: 1000000,        // 1 km² = 1e6 m²
    ha: 10000,           // 1 헥타르 = 1e4 m²
    in2: 0.00064516,     // 1 in² = 0.0254² m²
  ...
  function setActiveRows() {
    ...
    for (var i = 0; i < UNITS.length; i++) {
      ...
      if (r) r.className = (UNITS[i] === f || UNITS[i] === tt) ? "ac-active" : "";
    }
  }
  ...
  function render() {
    ...
    var sqm = num * FACTOR[from];
    for (var i = 0; i < UNITS.length; i++) {
      var u = UNITS[i];
      var c = $("cell-" + u);
      if (c) c.textContent = disp(sqm / FACTOR[u]);
    }
  }
  ...
  (function restoreUnits() {
    ...
    } catch (e) { /* 접근 불가·파싱 실패 — 기본 쌍(ft²→m²)으로 시작 */ }
  })();
