...
(function tool() {
  ...
  // TOOLJS:START
  var cfg = window.APP_CONFIG || {};
  var LS_KEY = (cfg.slug || "volume-conv") + ":units";

  // calc-core:start — 순수 계산 코어 (node 단위검증 대상, 리터 기준 환산 계수)
  ...
  function setActiveRows() {
    ...
    for (var i = 0; i < UNITS.length; i++) {
      ...
      if (r) r.className = (UNITS[i] === f || UNITS[i] === tt) ? "vc-active" : "";
    }
  }
  ...
  function render() {
    ...
    var liters = num * FACTOR[from];
    for (var i = 0; i < UNITS.length; i++) {
      var u = UNITS[i];
      var c = $("cell-" + u);
      if (c) c.textContent = fmt(liters / FACTOR[u]);
    }
  }
  ...
  (function restoreUnits() {
    ...
    } catch (e) { /* 접근 불가·파싱 실패 — 기본 쌍(gal US→L)으로 시작 */ }
  })();
