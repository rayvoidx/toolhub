...
(function tool() {
  ...
  // TOOLJS:START
  var cfg = window.APP_CONFIG || {};
  var LS_KEY = (cfg.slug || "energy-conv") + ":units";

  // calc-core:start — 순수 계산 코어 (node 단위검증 대상). 모든 환산은 joule(J)을 매개로.
  // cal = 소칼로리(열화학 칼로리), kcal = 영양표시 "Calorie"(대문자 C, 식품칼로리) = 1000 cal.
  // BTU = International Table BTU. ft·lb = foot-pound force.
  var FACTOR = {
    J: 1,
    kJ: 1e3,
    MJ: 1e6,
  ...
  // 표시 약어 = 표준 단위 기호 (언어 중립). id 가 곧 기호(ftlb 표기만 별도).
  function abbr(u) { return u === "ftlb" ? "ft·lb" : u; }
  ...
  function setActiveCells() {
    ...
    for (var i = 0; i < cells.length; i++) {
      ...
      if (u === f || u === tt) cells[i].classList.add("ec-active");
      else cells[i].classList.remove("ec-active");
    }
  }

  function clearTable() {
    for (var i = 0; i < UNITS.length; i++) {
      var c = $("ec-cell-" + UNITS[i]);
      if (c) c.textContent = "—";
    }
  }
  ...
  function render() {
    ...
    var joules = num * FACTOR[from];
    for (var i = 0; i < UNITS.length; i++) {
      var u = UNITS[i];
      var c = $("ec-cell-" + u);
      if (c) c.textContent = fmt(joules / FACTOR[u]);
    }
  }
  ...
  (function restoreUnits() {
    ...
    } catch (e) { /* 접근 불가·파싱 실패 — 기본 쌍(kWh→kJ)으로 시작 */ }
  })();
