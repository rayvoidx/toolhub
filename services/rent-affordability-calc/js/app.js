...
(function tool() {
  ...
  function calc() {
    var inc = num(income);
    if (blank(income) || isNaN(inc)) return fail("tool.err.empty");
    if (inc <= 0) return fail("tool.err.positive");

    // 빈 칸은 "부채 없음"으로 읽되, 글자가 들어있는데 못 읽으면 조용히 0 으로 넘기지 않는다.
    var d = blank(debts) ? 0 : num(debts);
    if (isNaN(d) || d < 0) return fail("tool.err.negative");

    var p = blank(people) ? 1 : num(people);
    ...
    // 임대료+부채 합계가 세전 소득의 40% 를 넘지 않게 — 부채가 많으면 30% 규칙보다 이 한도가 먼저 걸린다.
    var cap = monthly * 0.40 - d;
    if (cap <= 0) return fail("tool.err.debtload");

    var rent = Math.min(monthly * 0.30, cap);
    var cons = Math.min(monthly * 0.25, cap);
    var ceiling = annual / 40; // 집주인 심사선: 연소득 = 월세 x 40
    var left = monthly - rent - d;

    $("r-rent").textContent = money(rent);
    $("r-cons").textContent = money(cons);
    $("r-ceiling").textContent = money(ceiling);
    ...
    $("note-same").hidden = !(monthly * 0.30 <= cap && d === 0);
