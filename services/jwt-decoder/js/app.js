...
(function tool() {
  ...
  var input = $("jwt-input"), result = $("result"), errEl = $("err");
  if (!input) return;

  var t = function (k) { return (window.I18N && window.I18N.t) ? window.I18N.t(k) : k; };
  function fail(key) { result.hidden = true; errEl.hidden = false; errEl.textContent = t(key); }

  // base64url → 문자열. atob 는 표준 base64 만 받으므로 -_ 치환과 패딩 보정이 필요하다.
  function b64urlDecode(seg) {
    var s = String(seg).replace(/-/g, "+").replace(/_/g, "/");
    while (s.length % 4) s += "=";
    var bin = atob(s);
    // 비ASCII 클레임(이름 등)이 깨지지 않게 바이트로 되돌린 뒤 UTF-8 로 해석한다.
  ...
  function calc() {
    ...
    var parts = raw.split(".");
    if (parts.length !== 3 || !parts[0] || !parts[1]) return fail("tool.err.parts");
    ...
    $("out-header").textContent = JSON.stringify(header, null, 2);
    $("out-payload").textContent = JSON.stringify(payload, null, 2);
  ...
  Array.prototype.forEach.call(document.querySelectorAll(".copy-btn"), function (btn) {
    btn.addEventListener("click", function () {
      ...
      if (navigator.clipboard) navigator.clipboard.writeText(el.textContent).then(done, function () { /* 권한 거부 */ });
      else done();
    });
  });
