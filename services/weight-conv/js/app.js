...
(function tool() {
  ...
  // TOOLJS:START
  var cfg = window.APP_CONFIG || {};
  var LS_KEY = (cfg.slug || "weight-conv") + ":units";
  ...
  function setActiveRows() {
    ...
    for (var i = 0; i < UNITS.length; i++) {
      ...
      if (r) r.className = (UNITS[i] === f || UNITS[i] === tt) ? "wc-active" : "";
    }
  }
  ...
  function render() {
    ...
    var kilos = num * FACTOR[from];
    for (var i = 0; i < UNITS.length; i++) {
      var u = UNITS[i];
      var c = $("cell-" + u);
      if (c) c.textContent = fmt(kilos / FACTOR[u]);
    }
  }
  ...
  (function restoreUnits() {
    ...
    } catch (e) { /* 접근 불가·파싱 실패 — 기본 쌍(lb→kg)으로 시작 */ }
  })();
