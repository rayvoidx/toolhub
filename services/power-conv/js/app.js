...
(function tool() {
  ...
  // TOOLJS:START
  var cfg = window.APP_CONFIG || {};
  var LS_KEY = (cfg.slug || "power-conv") + ":units";

  // calc-core:start — 순수 계산 코어 (node 단위검증 대상, 와트(W) 기준 환산 계수)
  // 각 계수는 정의값에서 유도한 정확한 상수 — 반올림된 근사치가 아니다.
  //  ps    : 미터법 마력(PS/cv/hp(M)) = 75 kgf·m/s = 75 * 9.80665 W (정확)
  //  hp    : 기계식(영·미) 마력 = 550 ft·lbf/s (정의). ft·lbf = 0.3048 * 4.4482216152605 J
  //  btuh  : IT BTU/시간 = 1055.05585262 J / 3600 s (정확)
  ...
  if (typeof module !== "undefined" && module.exports) {
    module.exports = { FACTOR: FACTOR, UNITS: UNITS, convert: convert, fmt: fmt, plain: plain };
    return;
  }
  ...
  // fmt 가 null(비유한·오버플로) 이면 무한대 기호로 표시 — NaN/Infinity/빈칸 노출 금지
  function disp(v) {
    var s = fmt(v);
    return s == null ? "∞" : s;
  }
  ...
  function setActiveRows() {
    ...
    for (var i = 0; i < UNITS.length; i++) {
      ...
      if (r) r.className = (UNITS[i] === f || UNITS[i] === tt) ? "pc-active" : "";
    }
  }
  ...
  function render() {
    ...
    eqEl.textContent = disp(num) + " " + ABBR[from] + " = " + disp(convert(num, from, to)) + " " + ABBR[to];
    if (copyEl) copyEl.hidden = false;
    noteEl.hidden = true;

    var watts = num * FACTOR[from];
    for (var i = 0; i < UNITS.length; i++) {
      var u = UNITS[i];
      var c = $("cell-" + u);
      if (c) c.textContent = disp(watts / FACTOR[u]);
    }
  }
  ...
  (function restoreUnits() {
    ...
    } catch (e) { /* 접근 불가·파싱 실패 — 기본 쌍(hp→kW)으로 시작 */ }
  })();
