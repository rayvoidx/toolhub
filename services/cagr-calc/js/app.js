...
(function tool() {
  ...
  // TOOLJS:START
  var cfg = window.APP_CONFIG || {};
  var LS_KEY = (cfg.slug || "cagr-calc") + ":last";
  ...
  var v0El = $("v0-input"), vnEl = $("vn-input"), yearsEl = $("years-input");
  ...
  if (!v0El || !vnEl || !curSel || !calcBtn || !box) return;
  ...
  // ratio = Vn/V0 · CAGR = ratio^(1/n) − 1 · 총수익률 = ratio − 1 · 절대손익 = Vn − V0 · 성장배수 = ratio
  function computeCagr(v0, vn, n) {
    var clipped = false;
    if (v0 > LIM.value) { v0 = LIM.value; clipped = true; }
    if (vn > LIM.value) { vn = LIM.value; clipped = true; }
    if (n > LIM.years) { n = LIM.years; clipped = true; }
    var ratio = vn / v0;
    var cagr = Math.pow(ratio, 1 / n) - 1;   // Vn=0 → 0^(1/n)−1 = −1 (−100%, 전액 손실)
    return {
      ok: true, clipped: clipped, v0: v0, vn: vn, n: n,
      ratio: ratio, cagr: cagr, totalReturn: ratio - 1,
      absGain: vn - v0, multiple: ratio
    };
  }
  ...
  // 연도별 스케줄: k=1..floor(n) → V0·(1+CAGR)^k, 마지막 행은 정확한 n → Vn 로 마감
  function buildSchedule(v0, cagr, n, vn, maxRows) {
    var rows = [], full = Math.floor(n), truncated = false, lim = full, i;
    if (full > maxRows) { lim = maxRows; truncated = true; }
    for (i = 1; i <= lim; i++) rows.push({ year: i, value: v0 * Math.pow(1 + cagr, i) });
    if (n > full) {
      rows.push({ year: n, value: vn, fractional: true });      // 소수 기간: 정확한 n 으로 마감
    } else if (truncated) {
      rows.push({ year: full, value: vn, fractional: false });  // 정수 n·절삭: 마지막 정확행 보존
    } else if (rows.length) {
      rows[rows.length - 1].value = vn;                          // 정수 n: 부동소수 드리프트 제거
    }
  ...
  function calculate() {
    ...
    var v0 = parseAmount(v0El), vn = parseAmount(vnEl), yv = num(yearsEl);
    persist(cur);
    updateChips(yv);

    // 엣지케이스(철칙 5 — 전부 명시 처리)
    var periodEmpty = mode === "years" ? (yv == null) : (!startEl.value || !endEl.value);
    if (v0 == null || vn == null || periodEmpty) return showNotice("tool.err.empty", "Enter beginning value, ending value, and period.");
    if (v0 <= 0) return showNotice("tool.err.v0", "Beginning value must be greater than 0.");
    if (vn < 0) return showNotice("tool.err.vnNeg", "Ending value can't be negative.");

    var n, days = null;
    if (mode === "years") {
      if (yv <= 0) return showNotice("tool.err.period", "Period must be greater than 0.");
      n = yv;
    } else {
      var pr = periodFromDates(startEl.value, endEl.value);
      if (!pr.ok) {
        if (pr.reason === "order") return showNotice("tool.err.dateOrder", "End date must be after the start date.");
        return showNotice("tool.err.empty", "Enter beginning value, ending value, and period.");
      }
      n = pr.n; days = pr.days;
    }

    var r = computeCagr(v0, vn, n);
    errEl.hidden = true; bodyEl.hidden = false; box.hidden = false;

    var cagrEl = $("r-cagr");
    cagrEl.textContent = pctFmt(r.cagr, true);
    cagrEl.className = "rc-val " + (r.cagr >= 0 ? "pl-pos" : "pl-neg");
    var totEl = $("r-total");
    totEl.textContent = pctFmt(r.totalReturn, true);
    totEl.className = "rc-val " + (r.totalReturn >= 0 ? "pl-pos" : "pl-neg");
    var gainEl = $("r-gain");
    gainEl.textContent = moneySigned(r.absGain, cur);
    gainEl.className = "rc-val " + (r.absGain >= 0 ? "pl-pos" : "pl-neg");
    $("r-mult").textContent = multFmt(r.multiple);

    $("r-sub").textContent = t("tool.result.sub", "From {v0} to {vn} over {n} years.")
      .replace("{v0}", money(r.v0, cur)).replace("{vn}", money(r.vn, cur)).replace("{n}", numFmt(r.n, 4));

    var daysEl = $("r-days");
    if (mode === "dates" && days != null) {
      daysEl.hidden = false;
      daysEl.textContent = t("tool.result.daysNote", "{days} days between your dates ÷ 365.25 = {n} years.")
        .replace("{days}", numFmt(days, 0)).replace("{n}", numFmt(r.n, 4));
    } else { daysEl.hidden = true; }

    var lossEl = $("r-loss");
    if (r.vn === 0) {
      lossEl.hidden = false;
      lossEl.textContent = t("tool.result.totalLoss", "The ending value is 0 — a total loss, shown as −100%.");
    } else { lossEl.hidden = true; }

    var sc = buildSchedule(r.v0, r.cagr, r.n, r.vn, LIM.schedRows);
  ...
  function persist(cur) {
    try {
      localStorage.setItem(LS_KEY, JSON.stringify({
        v0: v0El.value, vn: vnEl.value, mode: radioVal("periodmode"),
  ...
  (function init() {
    ...
    if (saved) {
      if (saved.v0) v0El.value = saved.v0;
      if (saved.vn) vnEl.value = saved.vn;
    ...
    syncPeriodFields();
    var ready = parseAmount(v0El) != null && parseAmount(vnEl) != null;
  ...
  v0El.addEventListener("input", onAmountInput);
  vnEl.addEventListener("input", onAmountInput);
  ...
  [v0El, vnEl, yearsEl, startEl, endEl].forEach(function (el) { el.addEventListener("keydown", onEnter); });
