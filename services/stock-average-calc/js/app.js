...
(function tool() {
  ...
  // TOOLJS:START
  var cfg = window.APP_CONFIG || {};
  var LS_KEY = (cfg.slug || "stock-average-calc") + ":last";
  var SAFE = Number.MAX_SAFE_INTEGER;
  var LIM = { price: 1e12, qty: 1e9, pct: 100 };
  ...
  // 유효행 = q>0 AND p>=0. f=(sellFee+tax)/100, buyF=buyFee/100 (전부 raw number, 통화무관)
  ...
  function makeRow(qv, pv) {
    ...
    qi.placeholder = t("tool.lots.qtyPh", "Shares");
    qi.setAttribute("aria-label", t("tool.lots.qtyAria", "Shares"));
    ...
    pi.placeholder = t("tool.lots.pricePh", "Price");
    pi.setAttribute("aria-label", t("tool.lots.priceAria", "Price per share"));
  ...
  function relabelRows() {
    ...
    for (i = 0; i < rows.length; i++) {
      ...
      if (qi) { qi.placeholder = t("tool.lots.qtyPh", "Shares"); qi.setAttribute("aria-label", t("tool.lots.qtyAria", "Shares")); }
      if (pi) { pi.placeholder = t("tool.lots.pricePh", "Price"); pi.setAttribute("aria-label", t("tool.lots.priceAria", "Price per share")); }
  ...
  function calculate() {
    ...
    var pos = computePosition(readLots(),
      clampPct(num(buyfeeEl)), clampPct(num(sellfeeEl)), clampPct(num(selltaxEl)));

    if (!pos.ok) { showNotice("tool.err.noRows", "Add at least one buy (shares and price)."); persist(cur); updateChips(); return; }
    ...
    else $("r-breakeven").textContent = t("tool.err.breakEven", "Sell costs ≥100%, break-even not possible.");
    $("r-sub").textContent = t("tool.result.sub", "From {n} buys totaling {q} shares.")
      .replace("{n}", pos.valid).replace("{q}", qtyFmt(pos.Q));
    ...
    if (C != null && C <= 0) {
      ...
      if (pos.bePossible) {
        ...
        plNote.textContent = t("tool.result.plGross", "Before sell costs: {x}").replace("{x}", money(pl.grossPL, cur));
      } else {
        ...
        plNote.hidden = false; plNote.className = "subtle"; plNote.textContent = t("tool.result.plNoNet", "Sell costs ≥100% — net P/L not available; showing change before costs.");
      }
    } else {
  ...
  function updateChips() {
    var raw = selltaxEl.value.trim().replace(",", ".");
    var chips = document.querySelectorAll("#tax-chips .chip");
    for (var i = 0; i < chips.length; i++) {
      var dv = chips[i].getAttribute("data-tax");
  ...
  (function init() {
    ...
    if (saved) {
      ...
      if ((saved.buyFee && Number(saved.buyFee) > 0) || (saved.sellFee && Number(saved.sellFee) > 0) ||
          (saved.sellTax && Number(saved.sellTax) > 0) || (saved.current && Number(saved.current) > 0)) advEl.open = true;
  ...
  [buyfeeEl, sellfeeEl, selltaxEl, currentEl, planqtyEl, plantargetEl].forEach(function (el) {
  ...
  var taxChips = document.querySelectorAll("#tax-chips .chip");
  for (var tc = 0; tc < taxChips.length; tc++) {
    taxChips[tc].addEventListener("click", function () { selltaxEl.value = this.getAttribute("data-tax"); calculate(); });
  }
  calcBtn.addEventListener("click", calculate);
  function onEnter(e) { if (e.key === "Enter") calculate(); }
  [buyfeeEl, sellfeeEl, selltaxEl, currentEl, planqtyEl, plantargetEl].forEach(function (el) { el.addEventListener("keydown", onEnter); });
