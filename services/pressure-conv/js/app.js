...
(function tool() {
  ...
  // TOOLJS:START
  var cfg = window.APP_CONFIG || {};
  var LS_KEY = (cfg.slug || "pressure-conv") + ":units";

  // calc-core:start — 순수 계산 코어 (node 단위검증 대상, 파스칼(Pa) 기준 환산 계수)
  // psi/atm/mmHg/inHg 는 NIST SP811 관용값(정확 정의 또는 관용 정의)을 사용한다.
  var FACTOR = {
    pa: 1,
    kpa: 1000,
    mpa: 1000000,
  ...
  function showEmpty() {
    ...
    noteEl.hidden = false;
    noteEl.textContent = t("tool.result.placeholder", "Enter a number to see it in every pressure unit.");
    clearTable();
  }
  ...
  function render() {
    ...
    var pascals = num * FACTOR[from];
    for (var i = 0; i < UNITS.length; i++) {
      var u = UNITS[i];
      var c = $("cell-" + u);
      if (c) c.textContent = fmt(pascals / FACTOR[u]);
    }
  }
  ...
  (function restoreUnits() {
    ...
    } catch (e) { /* 접근 불가·파싱 실패 — 기본 쌍(psi→bar)으로 시작 */ }
  })();
